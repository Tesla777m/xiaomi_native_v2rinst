...
# Находим первую папку в /mnt/, имя которой начинается с nvme-
USB_DIR=$(find /mnt -maxdepth 1 -type d -name 'nvme-*' | head -n 1)

# Проверяем, что USB-папка найдена
if [ -z "$USB_DIR" ]; then
  echo "Директория /mnt/nvme-* не найдена в /mnt/"
  exit 1
fi
